...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Log.d("averageLessThanMedian output", averageLessThanMedian(listOf(2.0, 5.0, 9.3, 26.0, 30.0)).toString())
        Log.d("getView output", getView(0, null, listOf(3, 5, 7, 10), this).toString())
    ...
    private fun getView(position: Int, recycledView: View?, collection: List<Int>, context: Context): View =
        ...
        if (recycledView != null) {
            (recycledView as TextView).apply {text = collection[position].toString()}
        } else {
